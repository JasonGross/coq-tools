...
def filename_of_lib_helper(lib, non_recursive_libnames, ext):
    ...
    if len(filenames) != 0:
        DEFAULT_LOG(
            "WARNING: One or more physical paths match logical path %s, but none of them exist: %s (full list: %s)"
            % (lib, ", ".join(filenames), ", ".join(f"{physical_name} -> {logical_name}" for physical_name, logical_name in non_recursive_libnames)),
            level=LOG_ALWAYS,
        )
    if len(filenames) > 0:
        retval = filenames[0]
