...
def split_statements_to_definitions(
    ...
    **kwargs,
    ...
    proof_using_reg = re.compile(
        r"^<infomsg>\s*The proof of ([^\s]+) should start with one of the following commands: ([^<]+)</infomsg>".replace(
            " ", r"\s+"
        ),
    ...
    for i, prompt in enumerate(stdout.split("</prompt>")):
        ...
        if proof_using_match:
            ...
            proof_using_options = [s[:-1] if s.endswith(".") else s for s in proof_using_options]
