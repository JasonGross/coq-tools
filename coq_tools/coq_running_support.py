...
Goal False. admit. Qed."""
    errinfo = {}
    native_ondemand_args = list(
        get_coq_native_compiler_ondemand_fragment(coqc, coq_args=coqc_args, **kwargs)
    )
    for before, after in (
    ...
    raise Exception(
        "No valid ltac support snipped found.  Debugging info: %s" % repr(errinfo)
    )
...
def get_reserved_modnames(coqtop_prog, **kwargs):
    ...
    tokens = sorted(
        set(
            [i.strip('"') for i in re.findall(r'"[a-zA-Z_][^"]*"', grammars_output)]
            + COQ_GRAMMAR_TOKENS
        )
    )
    contents = "\n".join(
...
def filter_check_options_after_success(
    coqc_prog, contents: str, options: list, **kwargs
):
    if not options:
        return options
