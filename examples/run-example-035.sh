...
EXTRA_ARGS=("--faster-skip-repeat-edit-suffixes" "--no-minimize-args" "--no-try-all-inlining-and-minimization-again-at-end" --passing-coqc="${COQBIN}coqc" --coqc="${COQBIN}coqc" -R input Top --passing-R Passing Foo --nonpassing-R NonPassing Foo --no-deps "$@")
##########################################################
