...
if __name__ == "__main__":
    ...
    if len(search_for) < len(sys.argv[2]):
        ...
        p = subprocess.Popen(
            ["grep", "-o", "--color=auto", search_for], universal_newlines=False, stdin=subprocess.PIPE
        )
        p.communicate(input=b(sys.argv[1]))
        p.stdin.close()
        p.wait()
        ...
        print(
            "Mismatch: expected next characters at %d: %s"
            % (len(search_for), repr(sys.argv[2][len(search_for) :][:10]))
        )
        print(
            "Mismatch: actual next characters at %d: %s" % (len(stdout) - 1, repr(sys.argv[1][len(stdout) - 1 :][:10]))
        )
