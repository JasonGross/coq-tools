...
getting example_[0-9]\+\.glob
getting ../foo/bar/A.glob

Now, I will attempt to coq the file, and find the error\.\.\.

Coqing the file (bug_[0-9]\+\.v)\.\.\.

Running command: "coqc" "-Q" "\.\./foo/bar" "qux"\( "-top" "example_[0-9]\+"\)\?\( "-native-compiler" "ondemand"\)\? "/tmp/tmp[A-Za-z0-9_]\+\.v" "-q"
...
EXPECTED=$(cat <<EOF
(\* -\*- mode: coq; coq-prog-args: ("-emacs" "-Q" "\.\./foo/bar" "qux"\( "-top" "example_[0-9]\+"\)\?\( "-native-compiler" "ondemand"\)\?) -\*- \*)
